...
namespace Simple.Authorization.Auth
{
    public class PermissionProvider : AuthorizationProvider
    {
        public override void SetPermissions(IPermissionDefinitionContext context)
        {
            ...
            auth.CreateChildPermission(PermissionNames.Authorization_Admin, "管理列表", new PermissionMeta() { { "href", "/auth/admin" } });
            auth.CreateChildPermission(PermissionNames.Authorization_Role, "权限分组", new PermissionMeta() { { "href", "/auth/role" } });
        }
    }
}
